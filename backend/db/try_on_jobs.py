...
def create(connection: sqlite3.Connection, values: tuple[object, ...]) -> None:
    connection.execute("insert into try_on_jobs (id,user_id,person_path,garment_path,instructions,ratio,status,file_path,created_at) values(?,?,?,?,?,?,?,?,?)", values)
